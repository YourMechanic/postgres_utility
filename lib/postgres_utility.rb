...
module PostgresUtility
  ...
  DBLOGIN_FILE = Rails.root + "config/migrate_dblogin.yml"
  ...
  def has_pending_migration? # Copied from db:migrate:status
    db_list = rails_connection.select_values("SELECT version FROM #{ActiveRecord::Migrator.schema_migrations_table_name}")
  ...
  def create_database
    ...
    if is_postgresql?
      begin
        ...
        if e.to_s.include? "database \"#{db_name}\" already exists"
          return false
        end
        raise e
      end
  ...
  def copy_table_query(src_table_class, dest_table_class, opts = {})
    if !src_table_class.respond_to?(:table_name)
      raise StandardError.new("#{src_table_class} must be an ActiveRecord::Base Class")
    end
    if !dest_table_class.respond_to?(:table_name)
      raise StandardError.new("#{dest_table_class} must be an ActiveRecord::Base Class")
    end

    conn = rails_connection(opts[:cls])
    src_table  = src_table_class.table_name
    dest_table = dest_table_class.table_name
    if !conn.tables.include?(src_table)
      raise StandardError.new("#{src_table} Table missing!")
    end
    if !conn.tables.include?(dest_table)
      raise StandardError.new("#{dest_table} Table missing!")
    end
  ...
  def vacuum_analyze(tbl, opts = {})
    query = ""
    if tbl.respond_to?(:table_name) # Take care of the case where table is an ActiveRecord::Base class
      tbl = tbl.table_name
    end
  ...
  def multi_truncate_reset_populate_from_csv(tables, opts = {})
    ...
    tables.each do |table|
      ...
      if !tblcls.respond_to?(:table_name)
        raise StandardError.new("#{tblcls} must be an ActiveRecord::Base Class")
      end
    ...
    tables.each do |table|
      ...
      if !conn.tables.include?(tbl_copy)
        raise StandardError.new("#{tbl_copy} Table missing!")
      end
      tbl = tblcls.table_name
    ...
    if opts[:vacuum]
      tables.each { |t| vacuum_analyze(t[:tblcls], opts) }
    end
  end

  def fix_sequence_value(table, key = nil, c = nil)
    ...
    return unless c.select_value("SELECT sequence_name FROM information_schema.sequences WHERE sequence_name = '#{seqname}'")

    maxid = c.select_value("SELECT MAX(#{key}) from \"#{table}\"").to_i
    c.execute("SELECT setval('#{seqname}', #{maxid});") if maxid >= 1
  rescue
    print "WARNING: #{$!}\n"
  end

  def fix_sequence_value_with_cap(table, cutoff_id = nil, key = nil, c = nil)
    if table.respond_to?(:table_name) # Take care of the case where table is an ActiveRecord::Base class
      c ||= table.connection
      key ||= table.primary_key
      table = table.table_name
  ...
  def pg_dump_custom(filename, dbname = nil)
    dbname ||= db_name
    system_with_print("pg_dump #{host_param} -U #{username} -w --file=#{filename} -T data_parts -T data_part_applications --format=custom #{dbname}", "Dump #{dbname} to #{filename}")
  end

  def pg_save_data(filename, table_name, dbname = nil)
    dbname ||= db_name
    system_with_print("pg_dump #{host_param} -U #{username} -w -t #{table_name} --data-only --dbname=#{dbname} -f #{filename}", "Save #{dbname} to #{filename}")
  end

  def pg_restore_data_and_schema(filename, dbname = nil)
    dbname ||= db_name
    system_with_print("pg_restore #{host_param} -U #{username} -w -d #{dbname} -j 4 #{filename}", "Restore #{dbname} from #{filename}")
  end
  ...
  def pg_load(filename, dbname = nil)
    ...
    if bz2
      system_with_print("bzip2 -cd #{filename}.sql.bz2 | PGPASSWORD=\'#{password}\' psql #{host_param} -q -U #{username} -w #{dbname}", "Loading #{dbname} from #{filename}")
    else
      system_with_print("PGPASSWORD=\'#{password}\' psql #{host_param} -q -U #{username} -w #{dbname} < #{filename}", "Loading #{dbname} from #{filename}")
    end
  end

  # Much faster when it works, but does not work for foreign key enable tables
  def truncate_table(c, tbl)
    stmt = "TRUNCATE TABLE #{tbl}"
    c.execute(stmt)
  end

  def clear_table(c, tbl, cond=nil)
    if cond
      stmts = ["DELETE FROM #{tbl} WHERE #{cond}"]
    else
      stmts = ["DELETE FROM #{tbl}"]
    end
    stmts.each { |stmt| c.execute(stmt) }
  end
  ...
  def get_random_record(klass, opt=nil)
    opt ||= {}
    c = klass.connection
    cnt = c.select_value("SELECT " + ((opt[:dense]) ? "MAX(\"#{klass.primary_key}\")" : 'COUNT(*)') + " FROM \"#{klass.table_name}\"").to_i
    return klass.offset(rand(cnt)).first
  end
  ...
  def system_with_print(cmd, toprint=nil)
    ...
    start = Time.now
    ret = system(cmd)
    print (ret ? 'success' : 'failed' )+" (#{Time.now - start} seconds)\n"
    return ret
  end
  ...
  def username
    data       = File.exists?(DBLOGIN_FILE) ? YAML.load_file(DBLOGIN_FILE) : {}
    username   = data.fetch('login', {})&.fetch('username', nil)
  ...
  def password
    data       = File.exists?(DBLOGIN_FILE) ? YAML.load_file(DBLOGIN_FILE) : {}
    password   = data.fetch('login', {})&.fetch('password', nil)
    password ||= db_connection_config[:password]
    password
  end

  def generate_temptable_name(tbl_name)
    raise StandardError.new("Cant create temp table for missing original table") if tbl_name.blank?
